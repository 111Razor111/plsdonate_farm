--[[
    Pls Donate Farm Script v3.0 (Стабильная версия)
    Инжектор: Xeno
...
local function createScript()
    ...
    -- === Обработка донатов (УЛУЧШЕНО) ===
    local lastRaisedValue = 0
    local function setupDonationListener()
        -- 1. Отслеживание leaderstats (самый надежный метод)
    ...
    local function startJumpMode()
        ...
        updateStandText("Jump")  -- Обновляем текст на стенде
    ...
    local function shutdownScript()
        ...
        script:Destroy()  -- Полностью уничтожает скрипт
    end
    ...
    local function createGUI()
        ...
        title.Text = "Pls Donate Farm v3.0"
